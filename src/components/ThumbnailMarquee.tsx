    ...
    <div className="w-full mt-4  " >
       <FastMarquee speed={40} gradient={false} direction="right"   autoFill={true} className=""  >
        {thumbnail1.map((img,index)=>(
             <img key={index} src={img} alt={img} className="h-[30vh] w-full object-cover md:-[40vh] lg:h-[45vh] xl:h-[50vh] rounded-2xl p-1  " />
        ))}
        ...
        <FastMarquee speed={40} gradient={false} direction="left"   autoFill={true} >
            {thumbnail2.map((img,index)=>(
             <img key={index} src={img} alt={img} className="h-[30vh] w-full md:h-[40vh] lg:h-[45vh] xl:h-[50vh] p-1 rounded-2xl "  />
        ))}
